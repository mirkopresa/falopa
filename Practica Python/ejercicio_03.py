...
def perimetro_rectangulo(base: int, altura: int) -> int:
...
def area_rectangulo_v2(x_1: int, x_2: int, y_1: int, y_2: int) -> int:
    base = x_2 - x_1
    altura = y_2 - y_1
    area = base * altura
    return area
...
def perimetro_circulo(radio: int) -> int:
...
def volumen_esfera(radio: int) -> int:
...
def hipotenusa(a: int, b: int) -> int:
    hipotenusa = (a**2) + (b**2)
    return hipotenusa
